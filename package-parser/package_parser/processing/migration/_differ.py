...
def distance_elements(list_a: list[Any], list_b: list[Any], filter_function=lambda x: x) -> float:
    if len(list_a) == 0:
        return len(list_b)
    if len(list_b) == 0:
        return len(list_a)
    ...
    return 1 + min(distance_elements(list_a[1:], list_b), distance_elements(list_a, list_b[1:]), distance_elements(list_a[1:], list_b[1:]))
...
class SimpleDiffer(ABC):
    ...
    def diff_classes(class_a: Class, class_b: Class) -> float:
        ...
        attribute_diff = SimpleDiffer.diff_attributes(class_a.instance_attributes, class_b.instance_attributes)
    ...
    @staticmethod
    def diff_functions(function_a: Function, function_b: Function) -> float:
        diff_code = distance_elements(function_a.code.split("\n"), function_b.code.split("\n"))
        diff_name = SimpleDiffer.diff_names(function_a.name, function_b.name)
        diff_param = SimpleDiffer.diff_parameters(function_a.parameters, function_b.parameters)
    ...
    def diff_parameters(
        ...
        by_position_a, by_name_a, both_a, positional_vararg_a, named_vararg_a = SimpleDiffer.get_parameter_divided_by_assignment(parameter_a)
        by_position_b, by_name_b, both_b, positional_vararg_b, named_vararg_b = SimpleDiffer.get_parameter_divided_by_assignment(parameter_b)
        ...
        position_diff = distance_elements(by_position_a, by_position_b, filter_function=name_and_type_only)
        name_diff = distance_elements(by_name_a, by_name_b, filter_function=name_and_type_only)
        both_diff = distance_elements(both_a, both_b, filter_function=name_and_type_only)
        ...
        if positional_vararg_a is not None or positional_vararg_b is not None:
            total = total + 1
            position_vararg_diff = distance_elements([positional_vararg_a], [positional_vararg_b], filter_function=name_and_type_only)
        if named_vararg_a is not None or named_vararg_b is not None:
            total = total + 1
            named_vararg_diff = distance_elements([named_vararg_a], [named_vararg_b], filter_function=name_and_type_only)
        return (position_diff + name_diff + both_diff + position_vararg_diff + named_vararg_diff) / total

    @staticmethod
    def get_parameter_divided_by_assignment(parameter_list: list[Parameter]) -> Tuple[list[Parameter], list[Parameter], list[Parameter], Optional[Parameter], Optional[Parameter]]:
        by_position = []
        by_name = []
        ...
        return by_position, by_name, by_name_and_position, positional_vararg, named_vararg
