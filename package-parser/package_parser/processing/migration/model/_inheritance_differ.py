...
class InheritanceDiffer(AbstractDiffer):
    ...
    def compute_attribute_similarity(self, attributev1: Attribute, attributev2: Attribute) -> float:
        if attributev2.class_id in self.inheritance:
            for mapping in self.previous_mappings:
                if attributev1 in mapping.get_apiv1_elements() and attributev2 in mapping.get_apiv2_elements():
                    return self.differ.compute_attribute_similarity(attributev1, attributev2) + self.boost_value
        return 0.0
        ...
        if classv2.id in self.inheritance:
            for mapping in self.previous_mappings:
                if classv1 in mapping.get_apiv1_elements() and classv2 in mapping.get_apiv2_elements():
                    return self.differ.compute_class_similarity(classv1, classv2) + self.boost_value
        return 0.0
        ...
        if "/".join(functionv2.id.split("/")[:-1]) in self.inheritance:
            for mapping in self.previous_mappings:
                if functionv1 in mapping.get_apiv1_elements() and functionv2 in mapping.get_apiv2_elements():
                    return self.differ.compute_function_similarity(functionv1, functionv2) + self.boost_value
        return 0.0
    ...
    def get_related_mappings(self) -> Optional[list[Mapping]]:
        related_mappings = []
        mapped_apiv1_elements = [element for mapping in self.previous_mappings for element in mapping.get_apiv1_elements()]
        mapped_apiv2_elements = [element for mapping in self.previous_mappings for element in mapping.get_apiv2_elements()]
        for get_api_element in [lambda api: api.classes.values(), lambda api: api.functions.values(), lambda api: api.attributes(), lambda api: api.parameters(), lambda api: api.results()]:
            not_mapped_elements_mapping = self._get_not_mapped_api_elements(mapped_apiv1_elements, mapped_apiv2_elements, get_api_element)
