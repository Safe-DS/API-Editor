...
@pytest.mark.parametrize(
    ...
    [("", {}),
     (
         ...
         {'kind': 'UnionType',
          'types': [{'kind': 'EnumType', 'values': {'auto', 'log2', 'sqrt'}},
     ...
     (
         ...
         {'kind': 'UnionType',
          'types': [{'kind': 'NamedType', 'name': 'tuple of slice'},
     ...
     (
         ...
         {'kind': 'UnionType',
          'types': [{'kind': 'NamedType', 'name': 'ndarray'},
                    {'kind': 'NamedType', 'name': 'shape (n_samples,)'}]},
     ),
     ("estor adventus or None", {'kind': 'UnionType',
                                 'types': [{'kind': 'NamedType', 'name': 'estor adventus'},
                                           {'kind': 'NamedType', 'name': 'None'}]}),
     (
         "int or array-like, shape (n_samples, n_classes) or (n_samples, 1)                     when binary.",
         {'kind': 'UnionType',
          'types': [{'kind': 'NamedType', 'name': 'int'},
...
@pytest.mark.parametrize("docstring_type,expected",
                         [("Scale factor between inner and outer circle in the range `[0, 1)`",
                           {'base_type': 'float', 'kind': 'BoundaryType', 'max': 1.0, 'max_inclusive': False,
                            'min': 0.0, 'min_inclusive': True}),
                          (
                          "Tolerance for singular values computed by svd_solver == 'arpack'.\nMust be of range [1, infinity].\n\n.. versionadded:: 0.18.0",
                          {'base_type': 'float', 'kind': 'BoundaryType', 'max': 'Infinity', 'max_inclusive': True,
                           'min': 1.0, 'min_inclusive': True}),
                          ("", {})])
...
@pytest.mark.parametrize("docstring_type,docstring_description,expected",
                         [("int or 'Auto', or {'today', 'yesterday'}",
                           "base_type: int Scale factor between inner and outer circle in the range `[0, 10]`",
                           {'kind': 'UnionType',
                            'types': [{'base_type': 'float', 'kind': 'BoundaryType', 'max': 10.0, 'max_inclusive': True,
                                       'min': 0.0, 'min_inclusive': True},
                                      {'kind': 'EnumType', 'values': {'yesterday', 'today'}},
                                      {'kind': 'NamedType', 'name': 'int'}, {'kind': 'NamedType', 'name': "'Auto'"}]}),
                          ])
def test_boundary_and_union_from_string(docstring_type: str, docstring_description: str, expected: dict[str, Any]):
    assert Type(ParameterAndResultDocstring(docstring_type, docstring_description)).to_json() == expected
