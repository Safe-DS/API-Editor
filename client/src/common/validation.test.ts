import { decimalPattern, booleanPattern } from './validation';

test('valid natural number', () => {
    const testNumber = '1';
    expect(Boolean(testNumber.match(decimalPattern.value))).toEqual(true);
});
...
test('number with leading 0 is not a valid number', () => {
    ...
    expect(Boolean(testNumber.match(decimalPattern.value))).toEqual(false);
});
